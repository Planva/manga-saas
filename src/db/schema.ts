...
export const adminBannerSettingsTable = sqliteTable("admin_banner_settings", {
  ...
  messages: text({ mode: "json" }).notNull().$type<string[]>(),
  itemsPerCycle: integer().default(1).notNull(),
  bannerHeight: integer().default(36).notNull(),
});
export type AdminBannerSettings = InferSelectModel<typeof adminBannerSettingsTable>;
export const adminSystemSettingsTable = sqliteTable("admin_system_settings", {
  ...
  subsUnlimitedAlsoGrantCredits: integer().default(0).notNull(),
  dailyFreeCreditsEnabled: integer().default(1).notNull(),
  dailyFreeCredits: integer().default(10).notNull(),
  dailyFreeReset: integer().default(0).notNull(),
  perUseCreditCost: integer().default(1).notNull(),
  guestDailyFreeEnabled: integer().default(1).notNull(),
  guestDailyFreeCredits: integer().default(10).notNull(),
...
export const adminUserEventTable = sqliteTable("admin_user_event", {
  ...commonColumns,
  id: text().primaryKey().$defaultFn(() => `uevt_${createId()}`).notNull(),
  userId: text(),
  email: text(),
